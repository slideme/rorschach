...
function attemptLock(lock, timeout, callback) {
  ...
  function afterGetChildren(err, list) {
    ...
    if (result.error) {
      return onerror(result.error, true);
    }
...
function createNode(lock, callback) {
  ...
  var zk = lock.client.zk;

  zk.mkdirp(lock.basePath, afterCreateBase);

  function afterCreateBase(err) {
    if (err && err.getCode() !== Exception.NODE_EXISTS) {
      callback(err);
    }
    else {
      zk.create(nodePath, CreateMode.EPHEMERAL_SEQUENTIAL, callback);
    }
  }
}
...
Lock.prototype.release = function release(callback) {
  ...
  deleteNode(this, this.lockPath, done);
