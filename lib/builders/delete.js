...
function performDelete(builder, path, callback) {
  ...
  function exec(cb) {
    ...
    function handleResult(err) {
      if (err && err.getCode() === Exception.NOT_EMPTY && builder.deleteChildren) {
        utils.deleteChildren(builder.client.zk, path, true, after);
      }
      else {
        after(err);
      }
    ...
    function after(err) {
      if (err) {
        err = new ExecutionError('remove', [path, builder.version], err);
        cb(err);
      }
      else {
        cb();
