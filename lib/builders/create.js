...
function operationArguments(builder, path, data) {
  return [path, data, builder.acls, builder.mode];
}
...
CreateBuilder.prototype.forPath = function forPath(path, data, callback) {
  ...
  function afterCreate(err, nodePath) {
    ...
    function afterDelete(deleteErr) {
      if (deleteErr && !(deleteErr instanceof ExecutionError)) {
        var args = operationArguments(self, adjustedPath, data);
        deleteErr = new ExecutionError('create', args, deleteErr);
      }
